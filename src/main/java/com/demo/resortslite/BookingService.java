...
public class BookingService {
    ...
    // Credentials are now retrieved from AWS Secrets Manager at runtime with automatic rotation support
    @Value("${aws.secrets.database.name:resorts-db-credentials}")
    private String databaseSecretName;

    // FIXED cr-java-0069: Externalized payment API endpoint to environment variable
    ...
    public Map<String, Object> createBooking(String guestName, String roomType,
        ...
        // Using SHA-256 instead of MD5 for secure hashing
    ...
    public boolean authenticateUser(String username, String password) {
        try {
            GetSecretValueRequest request = GetSecretValueRequest.builder()
                    .secretId("resorts-user-credentials")
                    .build();
            GetSecretValueResponse response = secretsManagerClient.getSecretValue(request);
            String secretString = response.secretString();
            ...
            return username.equals(storedUsername) && sha256Hash(password).equals(storedPasswordHash);
        } catch (Exception e) {
            return false;
        }
    }
